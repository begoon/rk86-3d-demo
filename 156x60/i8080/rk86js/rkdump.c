...
int dump_file(const char* name) {
  ...
  // If the file name begins with "mon" and ends with ".mon",
  // we support that this is an image of Monitor (no sync-byte,
  // start and end addresses in front).
  if (!strcmp(period, ".bin")) {
    long sz;
